...
| 5 | **LLM/embedding cost control** | Every resume upload and job post triggers LLM calls, and the Apply Agent's scheduled scan is naively O(candidates × jobs) — combinatorial, repeated on every tick. Without controls this is not just a cost driver, it's a runaway-spend risk. | Full strategy in [02-ARCHITECTURE.md §7](02-ARCHITECTURE.md). Short version: most of the "AI pipeline" steps (salary/location/work-mode/experience checks) are deterministic code with zero LLM cost; content-hash caching means embeddings are computed once per resume/job version; a funnel (DB filter → vector search → rerank top-K → LLM generation only on human-visible views) keeps expensive calls bounded instead of combinatorial. |
